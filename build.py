...
TOC_CLASS = "post-toc"
# The toc extension emits the [TOC] as a <div>; it holds only a list, so the first </div> closes it.
TOC_DIV_RE = re.compile(rf'<div class="{TOC_CLASS}">(.*?)</div>', re.DOTALL)
...
def parse_post(path: Path) -> PostData:
    ...
    md = markdown.Markdown(
        extensions=[
            ...
            # plus a "#" permalink to copy (styled in blog.css, stripped from the feed).
            # A post opts into a table of contents by putting [TOC] on its own line.
            "toc",
        ...
        extension_configs={
            "toc": {
                "permalink": "#",
                "permalink_class": HEADING_ANCHOR_CLASS,
    ...
    html_body = TOC_DIV_RE.sub(
        rf'<nav class="{TOC_CLASS}" aria-label="Table of contents">\1</nav>', html_body
    )
