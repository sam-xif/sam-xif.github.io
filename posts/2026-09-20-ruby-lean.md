...
executable Lean model of Ruby's semantics, together with a proof of type
soundness of a small fragment of Sorbet's type system built on the semantics.
...
of programming languages and systems. Typically, these semantics are formulated
deep in research labs with multiple PhD-years of effort. The results make it to
academic conferences but don't find broad use in industry because 1) they often
have severe limitations, such as not being able to model the most complex parts
of a given language, and 2) authoring proofs against the semantics requires
technical expertise in formal methods and on the specific nature of the given
semantics. Advanced AI shows promise to address both of these impediments to
widespread utility of semantics.

As an outcome of this project, I am pleased to announce `ruby-lean`, my
semantics of [Ruby](https://www.ruby-lang.org/en/) in
[Lean 4](https://lean-lang.org/). `ruby-lean` is an executable semantics,
meaning that it can execute real Ruby code. It is modeled as a
...
**Both of the aforementioned limitations are effectively addressed:** the
semantics models complex features of Ruby like
...
1. Desugar[]^(In the "syntactic sugar" sense. Most programming languages can be projected to simpler subsets of themselves. The notion of "desugaring" was introduced by Krishnamurthi, Lerner, and Elberty in <a href="https://par.nsf.gov/servlets/purl/10124984"><i>The Next 700 Semantics: A Research Challenge</i></a>) the program, producing an s-expression that builds programs
...
cases via a different methodology. This is explained in more detail in the
[Technical Appendix](2026-09-20-ruby-lean-technical-appendix.html#phase-2-the-semantics).
...
1. **It has a *de facto* type checker.** Sorbet was created at Stripe, and it is
   ...
   soundness bug in Sorbet, where it claims a typed program is safe when it is
   not, would be immediately relevant to Ruby/Sorbet users.
...
1. ALWAYS have a clear idea of what you want the agent to do, unless you're
   ...
   agent gives you.
1. Having a ratchet discipline, where an agent can only make progress if some
   metric only goes up, is important.
