...
the string's `+` operator, which attempts to coerce using the `to_str` method,
with 2 as an operand," which then proceeds to fail as the code for `String#+`
executes because there is no `to_str` method on the `Integer` class. The
definition of meaning given by computational intent is known as a programming
language's
...
correct for any `x` that evaluates to a value (in other words, evaluating `x`
cannot result in an exception or non-termination). In Ruby, this program is
correct for `x` that can be coerced into a string (has a `to_str` method
defined). To make the Ruby program correct for all inputs, we can make the
`to_str` check explicit:
...
could still raise an exception or not terminate.
...
So, these example programs are dead simple, and yet they still have many edge
cases that are hard to reason about. This is why modeling the semantics of
entire programming languages has historically taken multiple PhD-years of
effort. [Mike Dodds](https://mikedodds.org/) proposed
...
Why? It's not because semantics are not inherently useful. Quite the opposite:
they have high potential utility because we can vary our definition of
correctness to suit the task at hand. They can be used, for instance, to prove
type safety, which has immediate business value by eliminating certain uncaught
exceptions in production code, and they can be used to prove robustness against
malicious inputs so that software can be relied on in security-critical
contexts.

Instead, semantics are not adopted because:

1. they often have severe limitations, such as not being able to model the
   complex parts of a given language which tend to be the most useful.
...
my semantics of Ruby in Lean 4. `ruby-lean` is an executable semantics, meaning
...
demonstrate that this semantics is *useful*, I have also developed a system of
type judgments based on Sorbet and proven it sound. *Soundness* means: if a
program passes the `ruby-lean` type validator, then it is completely free of a
certain family of type-related exceptions. **Both of the aforementioned
impediments to widespread use of this semantics are effectively addressed:** the
