...
How might you mount a mathematical argument of the correctness of a program
though? First, you need to define correctness. A simple definition is "this
program terminates and returns a value." Now, you need a way to encode
*meaning*. To see why, take the program `"2" + 2`. The meaning of this is
ambiguous because it depends on the programming language. If you've ever written
JavaScript, you may recognize that this program evaluates to `"22"`. In Ruby,
this program throws an exception because `2` cannot be automatically coerced
into a string. These outcomes hint at the *computational intent* of the
statement in each language. In JavaScript, the meaning is approximately
"concatenate the string '2' with the result of coercing 2 into a string." In
Ruby, the meaning is "attempt to call the string's `+` operator, which attempts
to coerce using the `to_str` method, with 2 as an operand," which then proceeds
to fail as the code for `String#+` executes. The definition of meaning given by
...
Armed with a definition of correctness and a semantics, we can consider a
program like `"2" + x`, where `x` is some input variable. In JavaScript, this
program is correct for any `x` that evaluates to a value (in other words,
evaluating `x` cannot result in an exception or non-termination). In Ruby, this
program is correct for `x` that can be coerced into a string (has a `to_str`
method defined). To make the Ruby program correct for all inputs, we can make
...
The astute reader might notice that even this is not correct! Evaluating `x`
could still raise an exception or not terminate. Termination is
[undecidable](https://en.wikipedia.org/wiki/Halting_problem) in general, but
reasoning about *type safety* can help with the possibility of exceptions. A
typical type checker, like [Sorbet](https://sorbet.org/) for Ruby, claims *if
your program passes, then the typed portions of the code never throw a type
error.* The type checker is *sound* if a successful type check implies that the
program truly is free of type errors. Unsoundness means the type checker can't
be trusted; it could accept a program that has type errors. A rigorous semantics
allows us to prove type soundness—another correctness property.
...
This is all well and good, but these example programs are dead simple, and yet
they still have edge cases that are hard to reason about. This is why modeling
the semantics of entire programming languages has historically taken multiple
PhD-years of effort. [Mike Dodds](https://mikedodds.org/) proposed
...
construction of useful semantics of programming languages and systems. I say
"useful" because results about semantics make it to academic conferences but
don't find broad use in industry. Why? 1) They often have severe limitations,
such as not being able to model the complex parts of a given language which tend
to be the most useful, and 2) authoring proofs against the semantics requires
technical expertise in formal methods and in the specific nature of the given
semantics. Advanced AI shows promise in addressing both of these impediments to
...
fellowship. As an outcome of this project, I am pleased to announce `ruby-lean`,
my semantics of [Ruby](https://www.ruby-lang.org/en/) in Lean 4. `ruby-lean` is
an executable semantics, meaning that it can execute real Ruby code. It is
modeled as a
[CESK machine](https://en.wikipedia.org/wiki/CEK_Machine#CESK_machine). I have
also developed a system of type judgments based on Sorbet and proven it sound. I
defined soundness previously, but in this concrete case it means: if a program
...
widespread use are effectively addressed:** the semantics models complex
features of Ruby like
...
1. Semantics *can* be built quickly (~2.5 months of part-time human labor +
   agents).
1. Semantics with AI-authored proof provide a foundation to scale correctness
...
1. **It's complicated.** Ruby has features that Python does not have that
   ...
   In Ruby, it's possible to pass a block to a function. A block is sort of like
   a lambda, except it has different scoping rules and multiple ways that it can
...
ratchet discipline I imposed in my most recent attempt at growing a sound type
system.
