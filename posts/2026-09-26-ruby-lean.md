...
correct for any `x` that can be coerced into a string (i.e., `x` has a `to_str`
method). To make the Ruby program correct for all inputs, we can make the
...
Why? It's not that semantics lack inherent utility. Quite the opposite: they
have high potential utility because we can vary our definition of correctness to
suit the task at hand. They can be used, for instance, to prove type safety,
which has immediate business value by eliminating certain uncaught exceptions in
production code, and they can be used to prove robustness against malicious
inputs so that software can be relied on in security-critical contexts.
