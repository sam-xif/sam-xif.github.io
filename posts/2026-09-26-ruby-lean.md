...
with. A typical type checker claims *if your program passes, then the typed
portions of the code never throw a type error.* The type checker is *sound* if a
successful type check implies that the program truly is free of type errors.
Unsoundness means the type checker can't be trusted; it could accept a program
that has type errors.
