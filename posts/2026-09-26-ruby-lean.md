...
correct for any `x` except for specific cases like `x = Symbol()`. In Ruby, this
...
[Sorbet](https://sorbet.org/), a widely used type checker for Ruby—accepts a
program, then the typed parts of the program are truly free of type errors. This
has immediate business value by provably ruling out certain uncaught exceptions
in production code. Semantics can additionally be used to prove robustness
against malicious inputs so that software can be relied on in security-critical
contexts.
...
discovered an instance of non-conformance via a differential testing campaign
related to the example Ruby program given in the
[intro](#introduction-semantics-done-quick). I have not yet patched this, so
while it is unpatched, you can run this program to convince yourself that CRuby
and `ruby-lean` are two distinct models of the language:
