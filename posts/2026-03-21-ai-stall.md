...
This all spells trouble for "thin wrapper"[]^(The notion of "wrappers" as a bad thing is debatable. SaaS historically was denigrated as "SQL wrappers" but it became a successful business model. SaaS is dying now though because the ease of building bespoke SQL wrappers has greatly increased.) AI startup companies that
...
The work will always be there for us to do, it's just a matter of where we draw
the line. With early AI pilots, this line shifted, perhaps too much. With each
advance of the AI reasoning frontier, this line shifts again. This process has
revealed that **humans are the bottleneck**. We are the organisms that have
intrinsic difficulty with context switching and parallel task management.
Computers have no such trouble because they have a fundamentally different
architecture.

We are now faced with a new type of problem, a human-computer interface problem.
If humans need to remain in the loop, how do we serialize AI output for human
consumption and interpretation in a way that is not cognitively overwhelming and
doesn't burn them out?

But what happens when humans don't need to remain in the loop? This isn't
possible at today's AI capability levels, but it might be in the future. The
more tasks AI can take on, the less touchpoints it needs to have with humans,
which means that there is potentially an exponential degree of
speedup.[]^(Consider a human that can handle a single task at a time. If their task has n subtasks, and each of those subtasks has n subtasks, then an AI that can achieve virtually infinite parallelism only needs to take three "units" of time, one for each layer of tasks, whereas a human would need n^2 + n + 1.)

This reality is achievable with future frontier reasoning capabilities, and it
will be the moment that humans are effectively liberated from labor. But at AI's
current capability levels, humans are stuck in the rut of needing to talk
serially to a fundamentally parallel machine. And if inference costs spike for
the best performing models, then most enterprises may not be able to afford to
pay their way out of this rut.

If employees under the current regime continue to experience cognitive
exhaustion, alternatively called
["AI Brain Fry"](https://hbr.org/2026/03/when-using-ai-leads-to-brain-fry) or as
@francedot on X calls it,
["Vibe Coding paralysis"](https://x.com/francedot/status/2017858253439345092),
and companies can't afford to throw money at expensive token generation, then
executives might start to realize negative return on investment into AI. With
61% of senior business leaders
...
to better AI) proves to be true, increased
reasoning will incur higher costs.
