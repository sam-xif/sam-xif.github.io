...
make inference cheaper, build in the open, and build community.
...
unrealistic. Firstly, I think 3.5x annual revenue growth is optimistic,
especially with the enterprise adoption question looming. If we adjust the
average revenue growth rate down to just 3.3x per year while keeping the cost
curve the same, OpenAI's revenue **never catches up to its costs** in this
model. The cost model also doesn't include other operating costs like headcount
and office space. Secondly, enterprises (and individual consumers for that
matter) will likely want the frontier of models which cost the most, so
...
Google, whose Gemini models have been making great strides lately. To provide an
anecdotal data point here, I have noticed that Claude Code will sometimes flash
a message saying something to the effect of *Opus models only outperform Sonnet
on complex planning tasks and cost 2x more.* I imagine there's a similar story
with OpenAI, Codex, and reasoning models that burn more compute per token and
more tokens as a whole during chain-of-thought rollout.
...
When inference gets more expensive,[*]^(But how much more expensive will inference get? That question remains open, and I am thinking of tackling that in a future post. Quantifying the potential cost increase will make this more tangible.) the tiered access pattern will
become manifest, and the pace of adoption will likely slow down due to stunted
growth in AI automation potential for the long tail of companies and humans.
Adoption won't fully stop; the capability of lower tier models will meet with
the automation potential of some percentile of tasks, and that market will be
cleared. But generalized job replacement will have to wait for stable energy and
...
in the status quo. However, there are many reasons why I could be wrong, and
I'll cover some in the next section.
