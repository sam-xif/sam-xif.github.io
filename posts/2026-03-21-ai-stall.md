...
inference costs, for multiple reasons, which will make all but the least
powerful models unaffordable for the average person and enterprise. This will
limit the potential for rapid societal and economic transformation, relegating
us to a reality where AI is another tool like email that usually works well but
can't automate anything in its own right. Some actionable things I believe we
can do to lower the probability of this outcome are:</i> create, research how to
make inference cheaper, open source everything, *and* build community.
...
frankly) a revolutionary spirit toward the end of this post with some thoughts
about what actions we can take to try to generate escape velocity from this
boring future. But to start, here are the main pillars of my argument:
...
There is nothing like a massive shift in focus when it comes to revealing the
points where a process is not robust. The AI buildout is one such shift in
focus. Human activity is inefficient because we can't predict the future. When
the economy is humming along, the probability distribution of what tomorrow will
look like is almost collapsed. Tomorrow will very likely look like today. The
probability distributions of 1 week, 1 month, 1 year out are not as collapsed,
and yet futures contracts to lock in oil prices are still traded. Some traders
will lose, some will win. That's the order of things.

Stripe's
[2025 annual letter](https://assets.stripeassets.com/fzn2n1nzq965/3LlGw839Q6kUwxZlLZDtH6/75ddcbada4aa7743dd8ec7d0f9ca497e/Stripe-annual-letter-2025-desktop.pdf)
discusses a similar idea that they dub the "Sorting Machine":

> At heart, competitive markets are a sorting machine. They direct profits,
> capital, and talent to the places of greatest impact, as determined by
> customers voting with their wallets. Historically, this sorting happened
> methodically. It typically took decades for a household name to be unseated or
> for a new entrant to reach meaningful scale.

> The sorting machine is now whirring faster: winners and losers are being
> anointed more quickly and more intensely. Today, the most profitable third of
> publicly listed companies in the US account for two-thirds of total market
> capitalization, the highest share since data began in 1963. And much of this
> is a story of profit concentration, not just valuations: the top 10% of the
> S&P 500 by market cap now account for roughly 59% of the index’s total
> profits, which is elevated relative to recent history
...
According to
[data](https://www.eia.gov/electricity/annual/table.php?t=epa_03_01_a.html) from
the US Energy Information Administration, total power generation at utility
scale factories was 4,309 TWh. News outlet
...
grow to 980 TWh in 2030. If we assume the 448 TWh figure for current data center
power demand is included in the 4,309 TWh total generation figure, that is a .1%
increase in total power generation to meet the data center demand for the LNBL
estimate, 12.3% for the Gartner estimate, and 14% for the high end of the
estimates given in the Word Resources Institute report.
...
than a traditional software as a service business. While both OpenAI and the
classic SaaS exemplar Salesforce are selling software, OpenAI has a much more
significant cost profile. Immense compute and intense R&D are not needed to
tweak a button on a page, but they are needed to produce GPT-6 and beyond and to
serve inference on those models to consumers. A utilities business charges by
usage, and it is a margin business: the profit is constrained to how much delta
betwee costs and revenue customers are willing to tolerate. When margin
businesses compete with each other, revenue gets compressed down to cost.

Importantly, usage-based services businesses have a very different margin
profile from SaaS businesses. And research and compute costs to train each
successive frontier mode have only been going up. Margins may be improving for
worse models, but the training and serving of frontier models that keeps their
business afloat is precisely what’s destroying their changes at profitability.
According to Epoch, the cost of training has been doubling every 8 months. The
cost of compute has not been halving every 8 months. This is, until the costs of
inference spike to cover COGS as well as R&D, or some other business structure
gets figured out. Perhaps the government needs to step in to find R&D.
...
company, so it is difficult to know the truth with certainty. At a minimum,
however, we can certainly say OpenAI is significantly unprofitable. Epoch AI
[estimates](https://epoch.ai/data-insights/openai-compute-spend) that OpenAI was
significantly unprofitable in 2024, making &dollar;3.7 billion revenue on
&dollar;1.8 billion inference spend and roughly &dollar;6 billion
total compute spend, including inference. This implies a plausible 50% gross
margin on inference, but a negative net margin that investors are covering.
...
every 8 months, or 2.7x per year. Let's see if we can put this all together with
some napkin math.
...
with AI becoming more involved in the code writing and research process itself),
then this simple model puts OpenAI's breakeven point in late 2028 (the X axis of
the following graph is years since 2024, Y axis is logarithmic scale of
revenue/cost).
...
years since 2024. This is roughly consistent with the figures Alex Salazar cites
in [his post](https://www.arcade.dev/blog/ai-inference-economics).

This model actually paints an optimistic picture for OpenAI, but I think it is
unrealistic.[]^(How unrealistic is it? What are OpenAI's expected compute costs, and how much more costly could compute become? I am thinking of tackling these questions in a future post where formalize this napkin math into something more compelling.) Firstly, I think 3.5x annual revenue growth is
optimistic, especially with the enterprise adoption question looming. If we
adjust the average revenue growth rate down to just 3.3x per year while keeping
the cost curve the same, OpenAI's revenue **never catches up to its costs** in
this model. The cost model also doesn't include other operating costs like
headcount and office space. Secondly, enterprises (and individual consumers for
that matter) will likely want the frontier of models which cost the most, so
maintaining a 50% inference margin becomes doubtful, especially in the face of
fierce competition from other frontier labs who can absorb costs, such as
Google, whose Gemini models have been making great strides lately.

To provide an anecdotal data point from my own experience, I have noticed that
Claude Code will sometimes flash a message saying something to the effect of
*Opus models only outperform Sonnet on complex planning tasks and cost 2x more.*
I imagine there's a similar story with OpenAI, Codex, and reasoning models that
burn more compute per token and more tokens as a whole during chain-of-thought
rollout.

### Trouble for GPT wrappers

This all spells trouble for "thin wrapper" AI startup companies that are
reselling marked up tokens from frontier providers.
...
writing on the wall and responded to it in two different ways. Windsurf accepted
acquisition by OpenAI before the inference cost crunch destroyed their margins.
Cursor took a different approach. They leveraged their data moat of real
transcripts of conversations between engineers and AI to train a domain-specific
coding model. This model is not good at other tasks, but is very good at coding,
and inference costs far less than frontier generalist models.
...
advantage. Training nonetheless requires compute, though, so the buildout
bottlenecks we explored in the previous section apply. There is still no free
lunch.

When inference gets more expensive, the tiered access pattern will become
manifest, and the pace of adoption will likely slow down due to stunted growth
in AI automation potential for the long tail of companies and humans. Adoption
won't fully stop; the capability of lower tier models will meet with the
automation potential of some percentile of tasks, and that market will be
cleared. Generalized job replacement will have to wait for stable energy and
compute inputs, efficiency gains in compute and model architecture, and
stabilizing inference costs.
